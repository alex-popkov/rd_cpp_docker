...
std::uint8_t to_msg_action(turret_controller::ActionStates action)
{
  switch (action) {
    case turret_controller::ActionStates::ACTION_IDLE: {
      return TurretStatus::ACTION_IDLE;
    }
    case turret_controller::ActionStates::ACTION_TRACK: {
      return TurretStatus::ACTION_TRACK;
    }
  }
  return TurretStatus::ACTION_IDLE;
}
...
public:
    ...
    : Node("turret_controller_node")
  {
    ...
    turret_status_pub_ = create_publisher<TurretStatus>(kTurretStatusTopic, 10);
...
private:
  ...
  rclcpp::Publisher<TurretStatus>::SharedPtr turret_status_pub_;
