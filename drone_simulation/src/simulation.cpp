...
{
  Coord position = {context.position.x, context.position.y};

  position.x += context.speed * cos(context.direction) * context.config->physicsTimeStep;
  position.y += context.speed * sin(context.direction) * context.config->physicsTimeStep;
...
float getAmmoHorizontalFlightDistance(const float& attackSpeed, const float& ammoFlightTime, const AmmoParams& ammo)
{
  ...
  float ammoHorizontalFlightDistance =
    attackSpeed * ammoFlightTime - pow(ammoFlightTime, 2) * ammo.drag * attackSpeed / (2 * ammo.mass) +
    pow(ammoFlightTime, 3) * (6 * ammo.drag * g * ammo.lift * ammo.mass - 6 * pow(ammo.drag, 2) * (pow(ammo.lift, 2) - 1) * attackSpeed) /
      (36 * pow(ammo.mass, 2)) +
    pow(ammoFlightTime, 4) *
      (-6 * pow(ammo.drag, 2) * g * ammo.lift * (1 + pow(ammo.lift, 2) + pow(ammo.lift, 4)) * ammo.mass +
       3 * pow(ammo.drag, 3) * pow(ammo.lift, 2) * (1 + pow(ammo.lift, 2)) * attackSpeed +
       6 * pow(ammo.drag, 3) * pow(ammo.lift, 4) * (1 + pow(ammo.lift, 2)) * attackSpeed) /
      (36 * pow(1 + pow(ammo.lift, 2), 2) * pow(ammo.mass, 3)) +
    pow(ammoFlightTime, 5) *
      (3 * pow(ammo.drag, 3) * g * pow(ammo.lift, 3) * ammo.mass -
       3 * pow(ammo.drag, 4) * pow(ammo.lift, 2) * (1 + pow(ammo.lift, 2)) * attackSpeed) /
...
DroneConfig readDroneConfig(const std::string& path)
{
  ...
  DroneConfig droneConfig = {
    .startPos = {.x = configJSON["drone"]["position"]["x"], .y = configJSON["drone"]["position"]["y"]},
    ...
    .physicsTimeStep = configJSON["simulation"].value("physicsTimeStep", 0.01f),
    .timeScale = configJSON["simulation"].value("timeScale", 10.0f),
  };
...
std::unordered_map<std::string, AmmoParams> getAmmoMap(json& ammoJSON)
{
  ...
  for (int i = 0; i < ammoCount; i++) {
    ...
    AmmoParams ammo = {.name = ammoName, .mass = ammoJSON[i]["mass"], .drag = ammoJSON[i]["drag"], .lift = ammoJSON[i]["lift"]};
...
std::vector<std::vector<Coord>> fillTargets(json& targetsJSON)
{
  ...
  for (int i = 0; i < targetsCount; i++) {
    ...
    for (int j = 0; j < timeSteps; j++) {
      track.push_back({targetsJSON["targets"][i]["positions"][j]["x"], targetsJSON["targets"][i]["positions"][j]["y"]});
    }
