...
struct BallisticTable {
    ...
    bool load(const std::string path) {
        ...
        axisZ0.resize(nZ); for (auto& v : axisZ0) dataFile >> v;
        axisV0.resize(nV); for (auto& v : axisV0) dataFile >> v;
        axisM.resize(nM);  for (auto& v : axisM)  dataFile >> v;
        axisD.resize(nD);  for (auto& v : axisD)  dataFile >> v;
        axisL.resize(nL);  for (auto& v : axisL)  dataFile >> v;
